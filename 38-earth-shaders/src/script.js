...
gui.add(earthParameters, 'cloudIntensity').name('Cloud intensity').min(0).max(1).onChange(() => {
    console.log(earthParameters.cloudIntensity)
    earthMaterial.uniforms.uCloudIntensity.value = earthParameters.cloudIntensity
})
...
// Tweaks
gui.add(sunSpherical, 'phi').name('Polar rotation').min(0).max(Math.PI).onChange(updateSun)
gui.add(sunSpherical, 'theta').name('Equator rotation').min(-Math.PI).max(Math.PI).onChange(updateSun)
